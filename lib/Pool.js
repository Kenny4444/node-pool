...
class Pool extends EventEmitter {
  ...
  _evict() {
    let loanResource = Array.from(this._resourceLoans.values());

    loanResource.forEach((item) => {
      const idleTime = Date.now() - item.pooledResource.lastBorrowTime;
      if (idleTime > this._config.idleTimeoutMillis) {
        this.release(item.pooledResource.obj);
      }
    });
